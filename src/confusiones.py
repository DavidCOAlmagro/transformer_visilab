...
def main() -> None:
    ...
    for i, especie_real in enumerate(nombres_clases):
        for j, especie_predicha in enumerate(nombres_clases):
            if i != j and matriz[i, j] >= UMBRAL_MINIMO:
                confusiones.append(
                    (especie_real, especie_predicha, int(matriz[i, j])))
    ...
    lineas: list[str] = [
        f"=== Confusiones con {UMBRAL_MINIMO} o más casos ==="]
    print(f"\n=== Confusiones con {UMBRAL_MINIMO} o más casos ===\n")
    for especie_real, especie_predicha, veces in confusiones:
        linea = f"{especie_real:35s} -> {especie_predicha:35s} : {veces} veces"
        print(linea)
        lineas.append(linea)

    ruta_salida.write_text("\n".join(lineas) + "\n", encoding="utf-8")
    print(f"\nSe ha guardado la salida en: {ruta_salida}")
